...
while IFS=" " read -r sample_name path; do
  ...
  ## data paths
  sample_dir=/home/abportillo/github_repo/RNA_seq_Bcell/scripts/raw_fastq_bcell/rnaPreprocess/${sample_name}
  mkdir -p /home/abportillo/github_repo/RNA_seq_Bcell/scripts/raw_fastq_bcell/rnaPreprocess/Transcript_finder
  ...
  {
  echo -e "#!/bin/bash
#SBATCH --job-name=${sample_name}_TEProf3
...
#SBATCH --output=${outdir}/${sample_name}_TEProf3.log
...
mamba deactivate
"
  } > ${outdir}/${sample_name}_runTEProf3.sh

  echo "Created script: ${outdir}/${sample_name}_runTEProf3.sh"
